...
def get_intra_data(symbol, tp):
    daily = yf.download(tickers=symbol, interval="5m",period=f"{str(tp)}d",end = dt.datetime(2021,2,23))
...
def get_dates(symbol, tp):
    daily = yf.download(tickers=symbol, interval="60m",period=f"{str(tp)}d",end = dt.datetime(2021,3,23))
